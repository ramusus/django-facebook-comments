...
from facebook_api.mixins import OwnerableModelMixin, AuthorableModelMixin, LikableModelMixin, CommentableModelMixin, ShareableModelMixin
from facebook_api.models import FacebookGraphIDModel, FacebookGraphManager, MASTER_DATABASE, Comment
from facebook_api.utils import graph, get_or_create_from_small_resource, UnknownResourceType
...
class Post(FacebookGraphIDModel, FacebookLikableModel):
    ...
    author_json = fields.JSONField(null=True, help_text='Information about the user who posted the message')
    ...
    story = models.TextField(
        help_text='Text of stories not intentionally generated by users, such as those generated when two users become friends; you must have the "Include recent activity stories" migration enabled in your app to retrieve these stories')
...
#        self.shares_count = instances.count()
    ...
    def fetch_shares(self, limit=1000, **kwargs):
        ...
        if response:
            ...
            timestamps = dict([(int(post['from']['id']), dateutil.parser.parse(post['created_time']))
            ...
            self.shares_users.get_query_set_through().filter(
                post_id=self.pk, user_id__in=map(lambda i: i[1], ids_add_pairs)).delete()

            # add new shares
            get_share_date = lambda id: timestamps[id] if id in timestamps else self.created_time
            m2m_model.objects.bulk_create(
                [m2m_model(**{'user_id': pk, 'post_id': self.pk, 'time_from': get_share_date(graph_id)}) for graph_id, pk in ids_add_pairs])
...
for Model in [Post]:
    Model.add_to_class('comments', generic.GenericRelation(
...
# object containing the name and Facebook id of the user who posted the message
#     author_json = fields.JSONField(null=True, help_text='Information about the user who posted the comment')
#
